...
def get_all_announcements() -> List[Dict[str, Any]]:
    """Get all announcements including expired ones (for management interface)"""
    return get_announcements(active_only=False)
...
def create_announcement(
    ...
    - expiration_date: When the announcement expires (ISO format)
    - start_date: Optional start date (ISO format). If None, starts immediately
    ...
    # Validate dates
    try:
        expiration = datetime.fromisoformat(expiration_date.replace('Z', '+00:00'))
